...
| 全屏 TUI            |            2.5 |                  3.5 |          5 | Focus 已全屏、差分、主题/伙伴，编辑器六组件起步；组件生态深度仍落后                   |
| 可爱助手/自定义外观 |            3.5 |                  4.5 |        3.5 | Focus 内置 6 只多状态伙伴并支持校验后的自定义 JSON                                    |
| 图片/多模态         |              3 |                  3.5 |        4.5 | Focus 四协议映射完整并新增剪贴板图片（未实测）；Pi 多了拖入和成熟终端体验             |
...
| Mid-turn queue      |              3 |                  4.5 |          5 | Focus append/interrupt/follow-up + 队列取回/delivery mode 已齐备；Pi 队列交互仍更成熟 |
...
| 全屏 TUI/主题/快捷键/卡通助手 | 全屏和内置主题/伙伴已有，但整帧刷新、不能安全加载自定义对象 | 差分刷新、title、自定义主题/多状态伙伴 JSON 校验；EditorBuffer（undo/kill ring/grapheme 光标）、tab 补全、EAW 宽字符、Markdown 渲染、diff 视图、剪贴板图片                    | **大部分关闭**：六组件已交付；IME preedit 终端协议不支持，剪贴板图片未真实 GUI 实测，编辑器深度与组件生态仍落后 Pi |
| 图片及多模态                  | 四协议和文件/URL已有                                        | 企业远程图片 egress policy、模型能力 Gate、模型级 image profile；macOS 剪贴板图片（/image，pngpaste/osascript）                                                               | **部分**：核心图片闭环完成；剪贴板图片已交付但未真实 GUI 实测；拖入、音视频未做                                    |
| 扩展分发与会话分享            | npm/签名分享已有，但企业运行时边界弱                        | 企业扩展 allowlist/签名/特权拒绝；share signer trust/TTL/auth/rate limit；进程外宿主模式（每扩展一子进程、崩溃隔离、60s 超时、env 白名单）                                    | **部分**：进程模式提供可靠性隔离与权限运行时强制挂点，不是安全沙箱；仍无市场、撤销服务和 HA 多租户存储             |
| Mid-turn steering             | append/interrupt 已有                                       | 新增 follow-up、RPC/TUI/SDK 一致、队列上限配置；队列取回（listSteering/unsteer、steering_removed、RPC unsteer/steering_list、TUI /unsteer）与 all/one-at-a-time delivery mode | **已关闭**：队列取回与 delivery 策略已交付                                                                         |
...
Extension 和 HMAC audit 接到 CLI/SDK 主路径，本轮再落地 spine 第一步：CodingAgent 可选注入
EffectPort，开启后工具调用走 Policy→Grant→Receipt，`tool_end` 元数据携带
grantId/receiptDigest 进入 HMAC 审计（`agent.effectSpine` 默认关闭）。但双规则源并存，
Policy 单源化仍是正式版前最大的架构债务之一。
...
扩展；此前允许列表内代码仍在主进程，本轮新增进程外宿主模式（每扩展一个子进程、崩溃隔离、
env 白名单、60s 工具超时），但这是可靠性隔离与权限运行时强制挂点，不是安全沙箱。最终方案
仍是 WASI/容器 + capability broker。
...
1. 在 spine 第一步（CodingAgent 可选 EffectPort）基础上完成 Policy 单源化，消除双主链；
2. 五类 Provider 的 recorded fixture + live smoke，固定 model revision 与兼容证书；
3. 真实 Docker/runsc/VM adversarial CI；
4. Session WAL/checksum/migration/repair 和 effect unknown-state reconciliation（跨实例恢复
   已测，kill-point 矩阵未做）；
5. Extension WASI/容器 capability sandbox（进程模式仅提供崩溃隔离，非安全 containment）；
6. OIDC/RBAC、workload identity、Secret/Egress Broker；
7. 同模型 Pi A/B 基准和 24h soak。

### P1：决定日常体验是否真正达到 Pi

1. 终端 editor 深化：selection/IME/hardware cursor（undo/kill ring/word movement 已交付）；
2. autocomplete 深化：`@file` fuzzy 与更广 provider（slash/skills/prompts/扩展命令/文件路径
   tab 补全已交付）；
3. Markdown/diff 已交付基础版；代码高亮、折叠 Tool/Thinking、完整 diff review UI 待做；
4. drag/drop（剪贴板图片、队列取回和 delivery mode 已交付）；
...
当前自动化证据：23 个 workspace project 构建，31 个测试文件、145 项测试，Statements 77.96%、
Branches 66.6%、Functions 83.55%、Lines 81.97%；architecture/schema 同步/format Gate 通过。
npm Gate 会执行 pack、洁净安装、文件 allowlist、已安装 CLI 版本/伙伴命令，以及本地 SSE
Provider 的两轮真实 Tool loop。
